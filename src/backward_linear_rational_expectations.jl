...
function add_static!(results::BackwardLinearRationalExpectationsResults,
    ...
    # B_s,s
    fill!(ws.b10, 0.0)
    @inbounds for i = 1:n_static(ws.indices)
        k = ws.static_indices_j[i]
        for j = 1:n_static(ws.indices)
    ...
    # C_s
    @inbounds for i = 1:n_backward(ws.indices)
        k = n_backward(ws.indices) + ws.current_nbr + i
        for j = 1:n_static(ws.indices)
            ws.temp1[j, i] = jacobian[j, k]
        end
    end
    # A_s
    for i = 1:n_backward(ws.indices)
        for j = 1:n_static(ws.indices)
...
function get_abc!(ws::LinearRationalExpectationsWs, jacobian::AbstractMatrix{Float64})
    ...
    col_s = n_backward(ws.indices) + ws.current_nbr + 1
    @inbounds for (i,j) in enumerate(ws.forward_indices_d)
        row_s = n_static(ws.indices) + 1
    ...
    @inbounds for (i,j) in enumerate(ws.indices.backward)
        row_s = n_static(ws.indices) + 1
        for k = 1:ws.dynamic_nbr
...
function get_de!(ws::LinearRationalExpectationsWs, jacobian::AbstractMatrix{Float64})
    n1 = n_backward(ws.indices) + ws.forward_nbr - ws.both_nbr
    fill!(ws.d, 0.0)
    fill!(ws.e, 0.0)
    for i = 1:length(ws.jcolsD)
        k = ws.jcolsD[i]
        for j = 1:ws.dynamic_nbr
            m = n_static(ws.indices) + j
...
function make_lu_AGplusB!(AGplusB, A, G, B, ws)
    ...
    for i = 1:ws.current_nbr
        k = ws.indices.current[i]
        for j = 1:n_endogenous(ws.indices)
            AGplusB[j, k] = B[j, i]
        end
    end
    for i = 1:n_backward(ws.indices)
    ...
    for i = 1:n_backward(ws.indices)
        k = ws.indices.backward[i]
        for j = 1:n_endogenous(ws.indices)
            AGplusB[j, k] += ws.temp7[j, i]
        end
    end
        #=
    vAGplusB = view(AGplusB, :, ws.indices.current)
    vAGplusB .= B
    ws.temp3 .= view(G, ws.forward_indices, :)
    mul!(ws.temp7, A, ws.temp3)
    vAGplusB = view(AGplusB, :, ws.indices.backward)
    vAGplusB .+= ws.temp7
        =#
...
function solve_for_derivatives_with_respect_to_shocks!(results::LinearRationalExpectationsResults, jacobian::AbstractMatrix, ws::LinearRationalExpectationsWs)
    ...
    if ws.exogenous_nbr > 0
        ...
        for i = 1:n_exogenous(ws.indices)
            k = ws.indices.exogenous_indices[i]
            for j = 1:n_endogenous(ws.indices)
...
function first_order_solver!(results::LinearRationalExpectationsResults,
    ...
    if algo == "CR"
        ...
        for i = 1:n_backward(ws.indices)
            for j = 1:n_backward(ws.indices)
                results.gs1[j, i] = ws.x[ws.indices.backward[j], ws.indices.backward[i]]
            end
            for j = 1:(n_endogenous(ws.indices) - n_static(ws.indices))
                results.g1[ws.dynamic_indices[j],i] = ws.x[j, ws.indices.backward[i]]
            end
        end
    elseif algo == "GS"
        get_de!(ws, jacobian)
        gs_solver!(ws.solver_ws, ws.d, ws.e, n_backward(ws.indices), options["generalized_schur"]["criterium"])
        ...
        for i = 1:n_backward(ws.indices)
            ...
            for j = 1:(ws.forward_nbr - ws.both_nbr)
                results.g1[ws.purely_forward_indices[j], i] =
                    ws.solver_ws.g2[ws.icolsE[n_backward(ws.indices) + j] - n_backward(ws.indices), i]
            end
        end
    else
        error("Algorithm $algo not recognized")
    end
    if n_static(ws.indices) > 0
